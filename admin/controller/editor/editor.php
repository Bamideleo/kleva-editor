...
class Editor extends Base {
	...
	function index() {
		// $theme              = $this->getTheme();
		//activate project base on user 
		$admin       = \Vvveb\System\User\Admin :: current();;
		$theme = $admin['project_name'] ?? 'landing';
		$newSlug = slugify($theme);
		// activate site 
	...
	function save() {
		...
		$admin = \Vvveb\System\User\Admin :: current();;
	$user_id = $admin['project_user'];
    $url = "https://api.mykleva.com/api/save-website";
$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https://" : "http://";
$host = $_SERVER['HTTP_HOST'];
$script = $_SERVER['SCRIPT_NAME'];
$parts = explode('/', trim($script, '/')); // ['Vvveb', 'admin', 'index.php']
$baseFolder = $protocol.$host.'/'. strtolower($theme);
      $details = [
            'link' => $baseFolder,
            'project_name' => strtolower($theme),   
      ...
      curl_setopt_array($curl, array(
        ...
        CURLOPT_POSTFIELDS => http_build_query($details), //<--Makes your array into application/x-www-form-urlencoded format.
